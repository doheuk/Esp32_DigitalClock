...
void setup() {
  ...
  attachInterrupt(digitalPinToInterrupt(Button1), btnIn1, FALLING);
  attachInterrupt(digitalPinToInterrupt(Button2), btnIn2, FALLING);
  attachInterrupt(digitalPinToInterrupt(Button3), btnIn3, FALLING);
  lcd.begin(20,4);
  bigNum.begin();
...
struct Time nowTime = {11,0,0};
struct Date nowDate = {2023,12,5,4};
bool btn1 = 0, btn2 = 0, btn3 = 0;
...
void mainScreen(){
  if(layout == 0) {
    ...
    if(nowTime.hour == 0)
      bigNum.displayLargeInt((nowTime.hour+12), 2, 2, false);
    else if(nowTime.hour < 13)
      bigNum.displayLargeInt(nowTime.hour, 2, 2, false);
    else
      bigNum.displayLargeInt(nowTime.hour%12, 2, 2, false);
    if(nowTime.min<10) {
      bigNum.displayLargeInt(0, 9, 1, false);
      bigNum.displayLargeInt(nowTime.min, 12, 1, false);
    } else
      bigNum.displayLargeInt(nowTime.min, 9, 2, false);
    lcd.setCursor(8,0);
    lcd.printf(".");
    lcd.setCursor(8,1);
    lcd.printf(".");
    lcd.setCursor(15,1);
    lcd.printf(".%d",nowTime.sec);
  }
  if(btn2) {
    lcd.clear();
    state = 8;
...
void timerScreen(){
  ...
  if(set_mode == 0) {
    if(btn2 && set_select == 5) {
      ...
      lcd.clear();
      timer2.detach();
      timerf1.hour = timerf1.min = timerf1.sec = 0;
      timer_start = 0;
